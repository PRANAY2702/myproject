...
import { ArrowRight, MapPin, Calendar, ChevronDown, Sparkles, Palette } from "lucide-react";
import APCLogo from "@/assets/images/APCLogoColor.jpg";
import CloudImg from "@/assets/images/Cloud.png";
import { pvsImages } from "@/app/gallery/helper";
import { faq_questions } from "@/app/home/faqs";
...
const PHOTOS = [
  { src: pvsImages[0], caption: "", rot: "-4deg", bg: "#fffbea", pin: "#e8c840" },
  { src: pvsImages[1], caption: "", rot: "3.5deg", bg: "#fff0f5", pin: "#e84060" },
  { src: pvsImages[2], caption: "", rot: "-2deg", bg: "#eef5ff", pin: "#4080e8" },
  { src: pvsImages[3], caption: "", rot: "4.5deg", bg: "#efffee", pin: "#30b840" },
  { src: pvsImages[4], caption: "", rot: "-3.5deg", bg: "#fff5ee", pin: "#e87030" },
  { src: pvsImages[5], caption: "", rot: "2deg", bg: "#f2eeff", pin: "#8040e0" },
];
...
export default function HomePage() {
  ...
  // NEW: Ref to track if user has manually changed the theme
  ...
  useEffect(() => {
    ...
    const tick = () => {
      const now = new Date();
      // Only auto-update the theme if the user hasn't overridden it
      if (!isManualTheme.current) {
  ...
  // NEW: Function to manually cycle through themes
  const cycleTheme = () => {
    isManualTheme.current = true; // Pause auto-updating for this session
    setTheme((prevTheme) => {
      const currentIndex = THEMES.findIndex(t => t.label === prevTheme.label);
  ...
  return (
    <>
      ...
      >
        {/* ── HERO ── */}
        <section className="relative flex flex-col items-center justify-center min-h-[100svh] px-4 md:px-8 pt-20 pb-20">
          <div className="flex flex-col items-center w-full max-w-5xl mx-auto text-center">
            ...
            <div className="fu3 max-w-2xl mx-auto">
              ...
              <h2
                className="text-[clamp(1.1rem,2.5vw,1.6rem)] font-bold tracking-wider mb-1"
      ...
      {/* ── NEW: Manual Theme Toggle Button (Fixed Bottom Right) ── */}
      <button
        onClick={cycleTheme}
        className="fixed bottom-6 right-6 z-[100] p-4 rounded-full border-[3px] border-[#08081a] bg-[#F6E245] shadow-[4px_4px_0px_#08081a] hover:-translate-y-1 hover:shadow-[6px_6px_0px_#08081a] active:translate-y-1 active:translate-x-1 active:shadow-[0px_0px_0px_#08081a] transition-all flex items-center justify-center group"
        title={`Current Theme: ${theme.label.toUpperCase()} (Click to change)`}
      >
        <Palette size={24} className="text-[#08081a] group-hover:rotate-12 transition-transform" />
      </button>
    </>
  );
}
